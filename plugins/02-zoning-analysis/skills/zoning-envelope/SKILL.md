...
**Requires a planning analysis report** generated by `/planning-analysis-wa` or `/zoning-analysis-uruguay`. This skill is a renderer — it does not perform planning calculations.

## Usage

```
/zoning-envelope path/to/planning-analysis.md
...
Search for matching planning analysis reports in these locations:
- `./` (WA)
- `./` (Uruguay)
...
Search for the most recently modified `planning-analysis-*.md`, `zoning-analysis-*.md` or `padron-*.md` file in the report directories. If found, confirm with the user. If not found, tell the user:

> No planning analysis report found. Run `/planning-analysis-wa` or `/zoning-analysis-uruguay` first, then come back with `/zoning-envelope`.
...
> This report was generated before the Envelope Data format was added. Re-run the planning analysis to get an updated report, or I can attempt to parse the tables (results may be approximate).
...
Scale camera distance to the lot's maximum dimension so it works for both small Uruguay lots (30m) and large suburban / city lots (60m+).
...
- `planning-analysis-48-swanbourne-st.md` → `zoning-envelope-48-swanbourne-st.html`

Open the file in the browser after saving.

## Notes

- **Dependency:** This skill requires a planning analysis report. It does not perform planning lookups, coordinate conversion, or regulation parsing — that's the analysis skill's job.
- **Units:** WA reports use metres; Uruguay uses metres. The `unit` field in the Envelope Data block determines all labels and scaling; imperial is still supported for legacy reports.
